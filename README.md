...
| 🔧 **SOP 執行引擎** | 三步驟選法規 → 版本 → 條件，自動載入參數、步驟自動確認、admin 手動接管模式、ISO 17025 CSV 報告下載 |
| 🗄️ **治具借還管理** | 借出 / 歸還 / 逾期追蹤、損壞遺失清單、月盤點、採購閉環、單筆新增 / 編輯 / 刪除、Excel 批次匯入（重複自動更新）、治具總表欄位排序 |
| 🤖 **AI 法規諮詢** | 自然語言查詢、RAG 法規檢索、多輪對話、右側欄常駐顯示 |
| 🗓️ **排程系統** | 甘特圖、自動排程（最早可用設備）、審核前即時預覽時段、不可用時段管理、自動避讓執行中設備 |

...
- 採購清單「無操作權限」：`purchase_orders.py` 讀 `request.state.role`（不存在），應為 `request.state.user_role`，兩處均修正
- `retryInTraditional` stale ref：重試時 `messagesRef.current` 未及時更新，history 包含舊訊息；修正：先同步更新 ref 再呼叫 `sendMessage`
...
- `estimated_replacement_date` 由 `_calc_replacement_date()` 即時計算（不新增 DB 欄位）
- APScheduler 每週一 08:00 執行 `scan_replacement_reminders()`，30 天內到期彙整推播保管人
- 治具總表「汰換」欄：顯示日期，30 天內標黃、已過期標紅
...
| ~~U7~~ | ~~Dashboard 卡片 vs SOPPage 按鈕組切換設備不一致~~ | ✅ 已修（v8） |

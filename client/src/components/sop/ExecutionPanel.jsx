...
const ExecutionPanel = ({
  ...
  const saveExecution = async () => {
    ...
    try {
      ...
      const url = `${API_BASE}/api/reports/csv/${execId}`;
      setReportUrl(url);
      onSaved(execId);

      // 自動觸發下載（瀏覽器可能攔截，備用連結同步顯示）
  ...
  // 已儲存狀態
  if (savedExecutionId) {
    const url = reportUrl || `${API_BASE}/api/reports/csv/${savedExecutionId}`;
    return (
      <div
        style={{
          marginTop: 12,
